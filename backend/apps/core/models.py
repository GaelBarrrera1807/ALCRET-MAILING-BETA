...
class Product(BaseModel):
    name = models.CharField(max_length=255, unique=True)
    description = models.TextField(blank=True, default='')
    sectors = models.ManyToManyField('core.Sector', related_name='products', blank=True, db_table='companies_product_sectors')
...
class Company(BaseModel):
    ...
    scraping_job = models.ForeignKey(
        'scraping.ScrapingJob', on_delete=models.SET_NULL,
        null=True, blank=True, related_name='companies',
    )
    google_rating = models.FloatField(null=True, blank=True)
    google_reviews_count = models.IntegerField(null=True, blank=True)
    maps_categories = models.JSONField(default=list, blank=True, null=True)
    opening_hours = models.JSONField(default=dict, blank=True, null=True)
    main_photo_url = models.URLField(max_length=500, null=True, blank=True)
